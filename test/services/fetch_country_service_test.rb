...
class FetchCountryServiceTest <ActiveSupport::TestCase
  test 'it should return ca with valid ip' do
    stub_request(:get, 'http://ip-api.com/json/24.48.0.1').
      to_return(status: 200, body: { status: 'success', countryCode: 'CA' }.to_json, headers: {})
    assert_equal(FetchCountryService.new('24.48.0.1').perform, 'ca')
  end

  test 'it should return nil with an ivalid ip' do
